...
[mid-FRET]->PPR Tightening [high-FRET]
[high-FRET] -> PPR Loosening[mid-FRET]
")
```
...
t1plot <- spr_coef %>%
  ...
  geom_errorbarh(aes(xmin = t1 - t1.err, xmax = t1 + t1.err),
                  height = 0.2) +
  geom_point(aes(x = t1, colour = rna)) +
  theme_light() +
  labs(x = "t1 (s)", y = "RNA") + 
  guides(colour = "none") + 
  xlim(c(0,NA)) + 
  ...
  theme(
    ...
    axis.text.y = ggtext::element_markdown(family = "mono", size = 6)
  )
...
t2plot <- spr_coef %>%
  ...
  geom_errorbarh(aes(xmin = t2 - t2.err, xmax = t2 + t1.err),
                  height = 0.2) +
  geom_point(aes(x = t2, colour = rna)) +
  theme_light() +
  labs(x = "t2 (s)", y = "RNA") + 
  guides(colour = "none") + 
  xlim(c(0,NA)) +
